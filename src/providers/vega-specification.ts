  ...
  public getVegaSpecification(chapterData:ChapterData[]) {
    ...
    return new Promise((resolve, reject) => {
      this.http.get('assets/data/vega_spec.json').subscribe(res => {
        ...
        //create a shallow copy, reverse the order
        chapterData = chapterData.slice().reverse();
        var cumulative = 0;
        ...
        if(chapterData.length > 0) {
          vegaData.unshift({"id": chapterData[0].id, "date": moment(chapterData[0].timestamp).subtract(5, "days").toDate(), "amount":0});
        }
